...
df = pd.read_csv('InP/doi_list.csv')
...
if st.button('Check your DOI'):
    if (df['DOI'] == DOI).any():
        ...
        doi_df.to_csv('InP/doi_list.csv', mode='a', header=False, index=False)
        st.markdown('**DOI list is updated**') 

st.write('Current DOI list:')
st.write(pd.read_csv('InP/doi_list.csv'))
...
if st.button('Submit Data'):
    user_df.to_csv('InP/InP_dataset.csv', mode='a', header=False, index=False)
    st.markdown('***Data submitted!***')

st.write('Current InP dataset:')
st.write(pd.read_csv('InP/InP_dataset.csv'))
st.markdown('****')
...
def get_table_download_link(df):
    ...
    href = f'<a href="data:file/csv;base64,{b64}" download="hot injection.csv">Download csv file</a>'
    return href
